...
def modeltry(file):
    ...
    # Define sets for indices and hubs
    I = np.arange(1, 82).tolist()  # Set of origin locations
    J = np.arange(1, 82).tolist()  # Set of destination locations
    K = [1, 6, 9, 10, 14, 16, 17, 20, 22, 27, 33, 34, 35, 39, 40, 41, 45, 51, 54, 59, 68, 77, 80, 81]  # Set of potential hub locations
    L = K.copy()  # Duplicate of K for hub-to-hub routes
    Q = 6  # Desired number of hubs
    alpha = 0.744  # Cost factor for inter-hub routes

    # Define possible routes and assignments
    Routes = [(i, j, k, l) for i in I for j in J for k in K for l in L]  # All possible hub-used routes
